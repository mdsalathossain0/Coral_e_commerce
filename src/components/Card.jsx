...
const Card = ({title, price, category, src }) => {
  ...
  return (
    <div className='w-[310px] h-[452px] relative group border border-[#0000001a] '>
        ...
        <Link to={`/productdetail/${title}`}><p className='text-black text-base font-semibold font-san px-4 pt-5 lg:pt-8'>{title}</p></Link>
        ...
        <div className='hidden lg:block cursor-pointer'>
          <div className='absolute left-0 bottom-0 bg-black w-full h-[55px] flex justify-between items-center px-5 opacity-0 group-hover:opacity-95 group-hover:bottom-[116px] duration-500'>
            ...
            <span onClick={handleClick} className='flex gap-x-3 items-center'><FaBagShopping className='text-white'/> <span className='text-white text-sm font-semibold font-san'>Shop Now</span></span>
        </div>
        </div>
